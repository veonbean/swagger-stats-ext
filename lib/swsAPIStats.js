...
swsAPIStats.prototype.countResponse = function (res) {
    ...
    // remove query params if statusCode is 200, otherwise the metrics will be more and more
    if (res.statusCode === 200) {
        req.sws.api_path = req.sws.api_path.replace(/\?.*/, '');
    }

    // request blacklist
    if (swsSettings.reqBlackList) {
        for (let i = 0; i < swsSettings.reqBlackList.length; i++) {
            if (req.sws.api_path.startsWith(swsSettings.reqBlackList[i])) {
                req.sws.api_path = swsSettings.reqBlackList[i] + '--';
                break;
            }
        }
    }
